...
def init():  # 将config文件内的信息读入到字典中
    f = open('D:/python/face-opencv/config.txt')
...
# 加载OpenCV人脸检测分类器Haar
face_cascade = cv2.CascadeClassifier('D:/python/face-opencv/haarcascade_frontalface_default.xml')
...
def write_config():
    print("新人脸训练结束")
    f = open('D:/python/face-opencv/config.txt', "a")
    T = Total_face_num
    ...
    f = open('D:/python/face-opencv/config.txt', 'r+')
    flist = f.readlines()
    flist[0] = str(int(flist[0]) + 1) + " \n"
    f.close()

    f = open('D:/python/face-opencv/config.txt', 'w+')
    f.writelines(flist)
    f.close()
